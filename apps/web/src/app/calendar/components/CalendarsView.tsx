/**
 * MODULE: Calendars List View Component
 *
 * Responsibility:
 * Displays the list of calendars with create, edit, and delete actions.
 *
 * Tags:
 * - domain: calendar
 * - risk: low
 * - layer: presentation
 * - stability: stable
 * - concerns: list-view, calendar-management
 *
 * File:
 * - apps/web/src/app/calendar/components/CalendarsView.tsx
 *
 * Last updated:
 * - July 23, 2026
 */

'use client';

import { Button, Badge } from '@life-os/ui';
import { Plus, Edit2, Trash2, Loader2 } from 'lucide-react';

import type { Calendar } from '../types';

interface CalendarsViewProps {
  calendars: Calendar[];
  calendarsLoading: boolean;
  onNewCalendar: () => void;
  onEditCalendar: (calendar: Calendar) => void;
  onDeleteCalendar: (id: string) => void;
}

export function CalendarsView({
  calendars,
  calendarsLoading,
  onNewCalendar,
  onEditCalendar,
  onDeleteCalendar,
}: CalendarsViewProps) {
  return (
    <div>
      <div className="flex justify-between items-center mb-4">
        <h2 className="text-xl font-semibold">Calendars</h2>
        <Button onPress={onNewCalendar}>
          <Plus className="w-4 h-4 mr-2" />
          New Calendar
        </Button>
      </div>

      {calendarsLoading ? (
        <div className="flex items-center justify-center p-8">
          <Loader2 className="w-6 h-6 animate-spin text-gray-400" />
        </div>
      ) : calendars.length === 0 ? (
        <div className="p-8 text-center border-2 border-dashed border-gray-300 rounded-lg bg-white">
          <p className="text-gray-600">
            No calendars yet. Create your first calendar to get started.
          </p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {calendars.map((calendar) => (
            <div
              key={calendar.id}
              className="p-6 bg-white border border-gray-200 rounded-lg hover:shadow-md transition-shadow group"
            >
              <div className="flex items-start justify-between mb-2">
                <div className="flex items-center gap-3">
                  {calendar.color && (
                    <div
                      className="w-3 h-3 rounded-full"
                      style={{ backgroundColor: calendar.color }}
                    />
                  )}
                  <h3 className="text-lg font-semibold">{calendar.name}</h3>
                </div>
                <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                  <Button variant="secondary" size="small" onPress={() => onEditCalendar(calendar)}>
                    <Edit2 className="w-3 h-3" />
                  </Button>
                  <Button
                    variant="secondary"
                    size="small"
                    onPress={() => onDeleteCalendar(calendar.id)}
                  >
                    <Trash2 className="w-3 h-3" />
                  </Button>
                </div>
              </div>
              {calendar.description && (
                <p className="text-gray-600 text-sm mb-4">{calendar.description}</p>
              )}
              <div className="flex gap-2">
                {calendar.isDefault && <Badge variant="info">Default</Badge>}
                {calendar.provider && <Badge variant="default">{calendar.provider}</Badge>}
              </div>
            </div>
          ))}
        </div>
      )}
    </div>
  );
}
