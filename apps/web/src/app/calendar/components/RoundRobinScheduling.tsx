...
interface TeamMemberCheckboxProps {
  member: TeamMember;
  isSelected: boolean;
  onToggle: (memberId: string) => void;
}

function TeamMemberCheckbox({ member, isSelected, onToggle }: TeamMemberCheckboxProps) {
  return (
    <label className="flex items-center gap-3 p-2 bg-white rounded border border-gray-200 cursor-pointer hover:bg-gray-50">
      <input
        type="checkbox"
        checked={isSelected}
...
interface RoundRobinEditFormProps {
  configName: string;
  selectedMembers: string[];
  distributionMethod: RoundRobinConfig['distributionMethod'];
  maxDailyBookings: number;
  bufferMinutes: number;
  teamMembers: TeamMember[];
  onConfigNameChange: (value: string) => void;
  onMemberToggle: (memberId: string) => void;
  onDistributionMethodChange: (value: RoundRobinConfig['distributionMethod']) => void;
  onMaxDailyBookingsChange: (value: number) => void;
  onBufferMinutesChange: (value: number) => void;
  onSave: () => void;
  onCancel: () => void;
}

function RoundRobinEditForm({
  configName,
  selectedMembers,
  distributionMethod,
  maxDailyBookings,
  ...
  onMaxDailyBookingsChange,
  onBufferMinutesChange,
  onSave,
  onCancel,
}: RoundRobinEditFormProps) {
  return (
    <div className="space-y-4 p-4 bg-gray-50 rounded-lg border border-gray-200">
      ...
      <div className="grid grid-cols-2 gap-4">
        <div>
          ...
          <Select
            ...
            onChange={(value) =>
              onDistributionMethodChange(value as RoundRobinConfig['distributionMethod'])
            }
        ...
        <div>
          ...
          <input
            ...
            onChange={(e) => onMaxDailyBookingsChange(parseInt(e.target.value, 10))}
            min={1}
            max={20}
            className="w-full px-3 py-2 border border-gray-300 rounded-md text-sm"
          />
      ...
      <div>
        ...
        <input
          type="number"
          value={bufferMinutes}
          onChange={(e) => onBufferMinutesChange(parseInt(e.target.value, 10))}
          min={0}
          max={60}
          className="w-full px-3 py-2 border border-gray-300 rounded-md text-sm"
        />
      ...
      <div className="flex gap-2">
        <Button onPress={onSave}>Save Configuration</Button>
...
interface RoundRobinViewModeProps {
  config: RoundRobinConfig;
  onEdit: () => void;
  onDelete: () => void;
}

function RoundRobinViewMode({ config, onEdit, onDelete }: RoundRobinViewModeProps) {
  return (
    <div className="space-y-4">
      <div className="p-4 bg-white rounded-lg border border-gray-200">
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="flex gap-2">
            <Button variant="secondary" size="small" onPress={onEdit}>
              Edit
            </Button>
        ...
        <div className="space-y-3">
          ...
          <div className="flex items-center gap-2 text-sm">
            ...
            <span className="text-gray-600">Max {config.maxDailyBookings} bookings per day</span>
          </div>
    ...
    <div className="space-y-4">
      ...
      {!config && !isEditing ? (
        ...
        <RoundRobinEditForm
          ...
          onMaxDailyBookingsChange={setMaxDailyBookings}
