import { pgTable, uuid, timestamp, text, jsonb, boolean, integer, index, customType } from 'drizzle-orm/pg-core';
...
export const tasks = pgTable('tasks', {
  ...
  workspaceIdProjectIdIdx: index('tasks_workspace_id_project_id_idx').on(table.workspaceId, table.projectId),
  projectIdIdx: index('tasks_project_id_idx').on(table.projectId),
  parentIdIdx: index('tasks_parent_id_idx').on(table.parentId),
  statusIdx: index('tasks_status_idx').on(table.status),
  dueDateIdx: index('tasks_due_date_idx').on(table.dueDate),
  workspaceIdStatusIdx: index('tasks_workspace_id_status_idx').on(table.workspaceId, table.status),
}));
