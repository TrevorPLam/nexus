...
enables JavaScript to run outside the browser, making it suitable for
server-side applications, CLI tools, and more.
...
Conditions (most to least specific): `"import"`, `"require"`, `"module-sync"`,
`"node"`, `"default"`
...
app.use(
  cors({
    origin: (origin, callback) => {
      ...
      callback(
        !origin || allowed.includes(origin) ? null : new Error('Not allowed'),
      );
    },
    credentials: true,
  }),
